# Section 2: Financial Snapshot - Research Prompt
...
```typescript
{
  ...
  fx_rates: Record<string, {rate: number; source: 'A' | 'B' | 'C'}>,
  industry_averages: {source: 'A' | 'B' | 'C'; dataset: string}
}
```
...
```json
{
  "confidence": {
    "level": "HIGH",
    "reason": "Recent 10-Q with detailed segment and geographic data; authoritative industry benchmarks"
  },
  "summary": "**German operations** generated €1.2B revenue (22% of global total) in FY2024, growing 15% YoY and outpacing company-wide growth of 12% (S1, S3). EBITDA margin expanded 80bps to 18.3% in the region, driven by pricing power and efficiency gains, running 90bps above the company average and significantly ahead of the 16.5% industry benchmark (S1, S7). DSO deteriorated 6 days YoY to 68 days, trapping approximately $50M in working capital compared to the peer average of 62 days (S1, S3). Operating cash flow of $850M represented a strong 15.5% of revenue, supporting $400M in growth CapEx focused primarily on German facility expansion (S1). **German facilities** operate at 84% capacity utilization versus 78% company average, with the Stuttgart plant achieving 92% OEE (S3, S5). FX rate source: A. Industry average source: A.",
  "kpi_table": {
    "metrics": [
      {
        "metric": "Revenue (Latest Period)",
        "company": "$5.5B",
        "industry_avg": "–",
        "source": "S1"
      },
  ...
  "derived_metrics": [
    ...
    {
      "metric": "ROIC",
      "formula": "NOPAT / Invested Capital where NOPAT = Operating Income × (1 - Tax Rate)",
      "calculation": "NOPAT = $1,010M × (1 - 0.22) = $788M; Invested Capital = $6,300M; ROIC = 12.5%",
      "source": "S1"
    }
  ],
  "sources_used": ["S1", "S3", "S5", "S7"]
}
```
