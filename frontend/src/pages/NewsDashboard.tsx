...
export const NewsDashboard: React.FC<NewsDashboardProps> = ({ onNavigate, isAdmin, currentUserId }) => {
  // Filters state - default to showing new (not archived or dismissed) articles
  ...
  const { status: deepDiveStatus, fetchStatus: fetchDeepDiveStatus } = useDeepDiveStatus();
  const { articles: deepDiveArticles, searchParams: deepDiveParams, fetchArticles: fetchDeepDiveArticles } = useDeepDiveArticles();
  ...
  // Poll deep dive status while searching
  useEffect(() => {
    if (!deepDiveStatus.isSearching) return;
    const interval = setInterval(fetchDeepDiveStatus, 1000);
    return () => clearInterval(interval);
  }, [deepDiveStatus.isSearching, fetchDeepDiveStatus]);

  // Refetch deep dive articles on completion
  useEffect(() => {
    if (deepDiveStatus.currentStep === 'complete' && !deepDiveStatus.isSearching) {
      fetchDeepDiveArticles();
    }
  }, [deepDiveStatus.currentStep, deepDiveStatus.isSearching, fetchDeepDiveArticles]);

  // Group articles by company or person name, sorted alphabetically
  const { companyGroups, personGroups } = useMemo(() => {
    const companyMap: Record<string, NewsArticle[]> = {};
    const personMap: Record<string, NewsArticle[]> = {};

    for (const article of articles) {
      if (article.company) {
        const name = article.company.name;
        if (!companyMap[name]) companyMap[name] = [];
        companyMap[name].push(article);
      } else if (article.person) {
        const name = article.person.name;
        if (!personMap[name]) personMap[name] = [];
        personMap[name].push(article);
      }
    }
    ...
    for (const key of Object.keys(companyMap)) companyMap[key].sort(sortByDate);
    for (const key of Object.keys(personMap)) personMap[key].sort(sortByDate);

    // Company names alphabetically
    const sortedCompanyNames = Object.keys(companyMap).sort((a, b) => a.localeCompare(b));

    // Person names sorted by last name
    const sortedPersonNames = Object.keys(personMap).sort((a, b) => {
      const lastA = a.split(' ').at(-1) || a;
      const lastB = b.split(' ').at(-1) || b;
      return lastA.localeCompare(lastB);
    });

    return {
      companyGroups: { groups: companyMap, sortedNames: sortedCompanyNames },
      personGroups: { groups: personMap, sortedNames: sortedPersonNames },
    };
  }, [articles]);
  ...
  // Execute the actual search (background deep dive)
  ...
  const handleDismiss = async (articleId: string) => {
    try {
      ...
      logger.error('Failed to dismiss article:', err);
    }
  };
  ...
  // Toggle selection of all articles in a group
  const toggleGroupSelection = (articleIds: string[], event: React.MouseEvent) => {
    event.stopPropagation();
    setSelectedArticleIds(prev => {
      const allSelected = articleIds.every(id => prev.has(id));
      const newSet = new Set(prev);
      if (allSelected) {
        for (const id of articleIds) newSet.delete(id);
      } else {
        for (const id of articleIds) newSet.add(id);
      }
      return newSet;
    });
  };

  // Handle bulk dismiss (admin only)
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center mb-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              disabled={deepDiveStatus.isSearching || resolving || (!searchCompany.trim() && !searchPerson.trim())}
          ...
          {/* Inline Deep Dive Status Bar */}
          {deepDiveStatus.isSearching && (
            <div className="mt-4 border-t border-slate-100 pt-4">
              <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-brand-50 to-violet-50 border border-brand-200 rounded-xl">
                <Loader2 className="animate-spin text-brand-500 flex-shrink-0" size={18} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-brand-700">{deepDiveStatus.progressMessage || 'Searching...'}</p>
                  <div className="w-full bg-brand-100 rounded-full h-1.5 mt-2 overflow-hidden">
                    <div
                      className="h-1.5 rounded-full bg-gradient-to-r from-brand-400 to-violet-500 transition-all duration-500 ease-out"
                      style={{ width: `${deepDiveStatus.progress}%` }}
                    />
                  </div>
                </div>
              </div>
          ...
          {/* Deep Dive Error */}
          {!deepDiveStatus.isSearching && deepDiveStatus.currentStep === 'error' && deepDiveStatus.lastError && (
            <div className="mt-4 border-t border-slate-100 pt-4">
          ...
          {/* Deep Dive Completion Message */}
          {!deepDiveStatus.isSearching && deepDiveStatus.currentStep === 'complete' && deepDiveArticles.length === 0 && (
            <div className="mt-4 border-t border-slate-100 pt-4">
              <div className="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                <CheckCircle2 className="text-emerald-500 flex-shrink-0" size={18} />
                <p className="text-sm text-emerald-700">Search complete &mdash; {deepDiveStatus.articlesFound} article{deepDiveStatus.articlesFound !== 1 ? 's' : ''} found</p>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 border-t border-slate-100 pt-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-slate-600">
                  <span className="text-brand-600 font-semibold">{deepDiveArticles.length}</span> results
                  {deepDiveParams && (
                    <span className="text-slate-400 ml-2">
                      for {[deepDiveParams.company, deepDiveParams.person].filter(Boolean).join(' & ')}
                    </span>
                  )}
                </span>
                <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              value={
                filters.isDismissed === true ? 'dismissed' :
                filters.isArchived === true ? 'archived' :
                (filters.isArchived === false && filters.isDismissed === false) ? 'new' :
                'all'
              }
              onChange={(e) => {
                const value = e.target.value;
                if (value === 'all') {
                  setFilters({ ...filters, isArchived: undefined, isDismissed: undefined });
                } else if (value === 'new') {
                  setFilters({ ...filters, isArchived: false, isDismissed: false });
                } else if (value === 'dismissed') {
                  setFilters({ ...filters, isArchived: undefined, isDismissed: true });
                } else if (value === 'archived') {
                  setFilters({ ...filters, isArchived: true, isDismissed: undefined });
                }
              }}
              className="w-full px-3 py-2.5 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-300 transition-all bg-slate-50 hover:bg-white cursor-pointer"
            >
      ...
      {/* News Sections - Grouped by Company, then Person */}
      {!articlesLoading && articles.length > 0 && (
        <div className="space-y-4">
          {companyGroups.sortedNames.map((name) => {
            const groupArticles = companyGroups.groups[name];
          ...
          {personGroups.sortedNames.map((name) => {
            const groupArticles = personGroups.groups[name];
            if (!groupArticles || groupArticles.length === 0) return null;
